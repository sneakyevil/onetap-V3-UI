...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos)//, float size)
{
    ...
    if (hovered)
    {
        float fMax = ImMax(2.0f, g.FontSize * 0.5f + 1.0f);
        window->DrawList->AddRectFilled(ImVec2(center.x - fMax, center.y - fMax), ImVec2(center.x + fMax, center.y + fMax), GetColorU32(ImGuiCol_TitleBg));
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawCornerFlags rounding_corners)
{
    ...
    // Render
    ImRect grab_rect;
    if (axis == ImGuiAxis_X) grab_rect = ImRect(ImLerp(bb.Min.x, bb.Max.x, grab_v_norm), bb.Min.y, ImLerp(bb.Min.x, bb.Max.x, grab_v_norm) + grab_h_pixels, bb.Max.y);
    ...
    if (window->Flags & ImGuiWindowFlags_Popup) GetForegroundDrawList()->AddRectFilled(grab_rect.Min, grab_rect.Max, grab_col, style.ScrollbarRounding);
    else window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, grab_col, style.ScrollbarRounding);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    if (!hovered) window->DC.LastItemStatusFlags &= ~ImGuiItemStatusFlags_HoveredRect;
    ...
    if (ImGuiWindow* popup_window = FindWindowByName(name))
        if (popup_window->WasActive)
        {
            ...
            r_outer.Max.y = FLT_MAX; // Ghetto Fix for combo to not fuck up 
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    else window->DC.LastItemStatusFlags &= ~ImGuiItemStatusFlags_HoveredRect;
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(cursor_screen_rect.GetBL() - ImVec2(0.f, 2.f), cursor_screen_rect.GetBL() + ImVec2(6.f, -2.f), GetColorU32(ImGuiCol_TitleBg));
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (InvisibleButton("##CBTN", ImVec2(30.f, 19.f)))
        {
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    float fHeightBG = 251.f;
    if (!alpha_bar) fHeightBG -= 30.f;
    draw_list->AddRectFilled(picker_pos, picker_pos + ImVec2(193.f, fHeightBG), GetColorU32(ImGuiCol_TitleBg), 3.f);
    draw_list->AddRectFilled(picker_pos + ImVec2(1.f, 1.f), picker_pos + ImVec2(192.f, fHeightBG - 1.f), IM_COL32(30, 35, 40, 255), 3.f);
    picker_pos.x += 11.f;
    picker_pos.y += 11.f;
    float bar0_pos_x = picker_pos.x + sv_picker_size + 9.f;
    float bar1_pos_x = bar0_pos_x + 20.f;
    ...
    if (IsItemActive())
    {
        S = 1.f - (ImSaturate((io.MousePos.x - picker_pos.x) / (sv_picker_size - 1)));
    ...
    InvisibleButton("hue", ImVec2(19.f, sv_picker_size));
    if (IsItemActive())
    {
    ...
    if (alpha_bar)
    {
        SetCursorScreenPos(ImVec2(picker_pos.x, picker_pos.y + sv_picker_size + 9.f));
        InvisibleButton("alpha", ImVec2(sv_picker_size + 28.f, 20.f));
        if (IsItemActive())
        {
            col[3] = ImSaturate((io.MousePos.x - picker_pos.x) / (sv_picker_size + 28.f));
            value_changed = true;
        }
    }
    ...
    if (!(flags & ImGuiColorEditFlags_NoSidePreview))
    {
        ...
        ImGuiColorEditFlags sub_flags_to_forward = ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_NoTooltip | ImGuiColorEditFlags_NoDragDrop;
        ImGui::SetCursorPos(ImVec2(18.f, alpha_bar ? 195.f : 165.f));
        ColorButton("##cnew", col_v4, (flags & sub_flags_to_forward), ImVec2(87.f, 22.f));
        const char* cNew = "new";
        float fTextCenter = 61.f - (CalcTextSize(cNew).x * 0.5f);
        ImGui::SetCursorPos(ImVec2(fTextCenter, alpha_bar ? 196.f : 166.f));
        Text(cNew);
        if (ref_col != NULL)
        {
            ImVec4 ref_col_v4(ref_col[0], ref_col[1], ref_col[2], (flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : ref_col[3]);
            ImGui::SetCursorPos(ImVec2(103.f, alpha_bar ? 195.f : 165.f));
            if (ColorButton("##cold", ref_col_v4, (flags & sub_flags_to_forward), ImVec2(86.f, 22.f)))
            {
                memcpy(col, ref_col, components * sizeof(float));
                value_changed = true;
            }
            const char* cOld = "old";
            fTextCenter = 146.f - (CalcTextSize(cOld).x * 0.5f);
            ImGui::SetCursorPos(ImVec2(fTextCenter, alpha_bar ? 196.f : 166.f));
            Text(cOld);
        }
        PopItemFlag();
    ...
    // Render SV Square
    draw_list->AddRectFilledMultiColor(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), hue_color32, col_white, col_white, hue_color32);
    draw_list->AddRectFilledMultiColor(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), 0, 0, col_black, col_black);
    RenderFrameBorder(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), 0.0f);
    sv_cursor_pos.x = ImClamp(IM_ROUND(picker_pos.x + ImSaturate(1.f - S) * sv_picker_size), picker_pos.x + 2, picker_pos.x + sv_picker_size - 2); // Sneakily prevent the circle to stick out too much
    sv_cursor_pos.y = ImClamp(IM_ROUND(picker_pos.y + ImSaturate(1 - V) * sv_picker_size), picker_pos.y + 2, picker_pos.y + sv_picker_size - 2);

    // Render Hue Bar
    for (int i = 0; i < 6; ++i)
        draw_list->AddRectFilledMultiColor(ImVec2(bar0_pos_x + 1.f, picker_pos.y + i * (sv_picker_size / 6)), ImVec2(bar0_pos_x + 20.f, picker_pos.y + (i + 1) * (sv_picker_size / 6)), col_hues[i], col_hues[i], col_hues[i + 1], col_hues[i + 1]);
    float bar0_line_y = IM_ROUND(picker_pos.y + H * sv_picker_size);
    RenderFrameBorder(ImVec2(bar0_pos_x + 1.f, picker_pos.y), ImVec2(bar0_pos_x + 19.f, picker_pos.y + sv_picker_size), 0.0f);
    draw_list->AddRectFilled(ImVec2(bar0_pos_x, bar0_line_y - 1.f), ImVec2(bar0_pos_x + 20.f, bar0_line_y + 3.f), IM_COL32(30, 35, 40, 255));
    draw_list->AddRect(ImVec2(bar0_pos_x, bar0_line_y - 1.f), ImVec2(bar0_pos_x + 20.f, bar0_line_y + 3.f), IM_COL32_WHITE);

    // Render cursor/preview circle (clamp S/V within 0..1 range because floating points colors may lead HSV values to be out of range)
    draw_list->AddCircle(sv_cursor_pos, 6.f, col_white, 12);

    // Render alpha bar
    if (alpha_bar)
    {
        float alpha = ImSaturate(col[3]);
        ImRect bar1_bb(picker_pos.x, picker_pos.y + sv_picker_size + 10.f, picker_pos.x + sv_picker_size + 29.f, picker_pos.y + sv_picker_size + 28.f);
        RenderColorRectWithAlphaCheckerboard(draw_list, bar1_bb.Min, bar1_bb.Max, 0, 5.f, ImVec2(0.f, -2.f));
        ...
        float fCenter = ((bar1_bb.Max.x - 2.f) - (bar1_bb.Min.x + 2.f)) * col[3];
        draw_list->AddRectFilled(ImVec2(bar1_bb.Min.x + fCenter, bar1_bb.Min.y - 1.f), ImVec2(bar1_bb.Min.x + fCenter + 4.f, bar1_bb.Max.y), IM_COL32(30, 35, 40, 255));
        draw_list->AddRect(ImVec2(bar1_bb.Min.x + fCenter, bar1_bb.Min.y - 1.f), ImVec2(bar1_bb.Min.x + fCenter + 4.f, bar1_bb.Max.y), IM_COL32_WHITE);
    }

    float f[4] = { col[0], col[1], col[2], alpha_bar ? col[3] : 1.0f };
    int i[4] = { IM_F32_TO_INT8_UNBOUND(f[0]), IM_F32_TO_INT8_UNBOUND(f[1]), IM_F32_TO_INT8_UNBOUND(f[2]), IM_F32_TO_INT8_UNBOUND(f[3]) };
    char buf[64];
    if (alpha_bar)
        ImFormatString(buf, IM_ARRAYSIZE(buf), "#%02X%02X%02X (%d)", ImClamp(i[0], 0, 255), ImClamp(i[1], 0, 255), ImClamp(i[2], 0, 255), ImClamp(i[3], 0, 255));
    else
        ImFormatString(buf, IM_ARRAYSIZE(buf), "#%02X%02X%02X", ImClamp(i[0], 0, 255), ImClamp(i[1], 0, 255), ImClamp(i[2], 0, 255));
    draw_list->AddRectFilled(picker_pos + ImVec2(20.f, alpha_bar ? 208.f : 178.f), picker_pos + ImVec2(151.f, alpha_bar ? 229.f : 199.f), IM_COL32(45, 50, 55, 255), 3.f);
    float fColorInfoCenter = 105.f - CalcTextSize(buf).x * 0.5f;
    SetCursorPos(ImVec2(fColorInfoCenter, alpha_bar ? window->Pos.y - picker_pos.y + 247.f : window->Pos.y - picker_pos.y + 217.f));
    Text(buf);
    SetCursorPos(ImVec2(10.f, 10.f));
    InvisibleButton("##CBounding", ImVec2(187.f, alpha_bar ? 242.f : 212.f));
    /*Text(" "); // Fix bounding kinda ghetto*/
    EndGroup();
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    const ImVec2 text_min(ImClamp(pos.x, window->Pos.x, window->Pos.x + window->Size.x), ImClamp(pos.y, window->Pos.y, window->Pos.y + window->Size.y - 5.f));
    const ImVec2 text_max(ImClamp(min_x + size.x, window->Pos.x, window->Pos.x + window->Size.x), ImClamp(pos.y + size.y, window->Pos.y, window->Pos.y + window->Size.y - 5.f));
    if (10.f >= text_max.y - text_min.y) return false;
    
    float fTextSize = CalcTextSize(label).x;
    float fTextCenter = static_cast<float>(((text_max.x - text_min.x) * 0.5f) - (fTextSize * 0.5));

    // Selectables are meant to be tightly packed together with no click-gap, so we extend their box to cover spacing between selectable.
    ImRect bb(text_min.x + fTextCenter, pos.y, text_min.x + fTextCenter + fTextSize, text_max.y);
    ...
    const bool was_selected = selected;
    bool hovered, held;
    const float fOldMaxY = bb.Max.y;
    bb.Max.y = text_max.y - 5.f;
    ...
    if (selected || hovered) PushStyleColor(ImGuiCol_Text, style.Colors[ImGuiCol_TitleBg]);
    ImGuiWindow* pWindow = GetCurrentWindow();
    ImDrawList* pOldDrawList = pWindow->DrawList;
    const bool bIsCombo = pWindow->Flags & ImGuiWindowFlags_Popup;
    if (bIsCombo) pWindow->DrawList = GetForegroundDrawList();
    RenderTextClipped(text_min + ImVec2(fTextCenter, -5.f), text_max - ImVec2(0.f, 5.f), label, NULL, &label_size, style.SelectableTextAlign, &bb);
    if (bIsCombo) pWindow->DrawList = pOldDrawList;
