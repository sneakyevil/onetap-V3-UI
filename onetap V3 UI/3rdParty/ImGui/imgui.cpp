...
void ImGui::UpdateMouseMovingWindowNewFrame()
{
    ...
    if (g.MovingWindow != NULL)
    {
        ...
        if (g.IO.MouseDown[0] && IsMousePosValid(&g.IO.MousePos))
        {
            ...
            {
                if (g.IO.MousePos.y >= pos.y && pos.y + 25.f >= g.IO.MousePos.y)
                {
                    if (0.f > pos.x) pos.x = 0.f;
                    else if (pos.x + moving_window->Size.x > g.IO.DisplaySize.x) pos.x = g.IO.DisplaySize.x - moving_window->Size.x;

                    if (0.f > pos.y) pos.y = 0.f;
                    else if (pos.y + moving_window->Size.y > g.IO.DisplaySize.y) pos.y = g.IO.DisplaySize.y - moving_window->Size.y;
...
bool IsMainMenu = false, IsChildMenu = false, IsNavBar = false;
const char* cNavBar;
void ImGui::RenderWindowDecorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size)
{
    ...
    else
    {
        if (IsNavBar)
        {
            ...
            float fCenter = (title_bar_rect.Max.x - title_bar_rect.Min.x) * 0.5f;
            window->DrawList->AddRectFilledMultiColor(title_bar_rect.Min, ImVec2(title_bar_rect.Min.x + fCenter, title_bar_rect.Min.y + 2.f), ImGui::GetColorU32(ImGuiCol_TitleBg), iTop32, iTop32, ImGui::GetColorU32(ImGuiCol_TitleBg));
            window->DrawList->AddRectFilledMultiColor(ImVec2(title_bar_rect.Min.x + fCenter, title_bar_rect.Min.y), ImVec2(title_bar_rect.Max.x, title_bar_rect.Min.y + 2.f), iTop32, ImGui::GetColorU32(ImGuiCol_TitleBg), ImGui::GetColorU32(ImGuiCol_TitleBg), iTop32);
            if (cNavBar[0] != '\0')
            {
                window->DrawList->AddRectFilled(ImVec2(title_bar_rect.Min.x, title_bar_rect.Min.y + 2.f), ImVec2(title_bar_rect.Max.x, title_bar_rect.Min.y + 20.f), IM_COL32(0, 0, 0, 30));
                float fTextCenter = (title_bar_rect.Max.x - title_bar_rect.Min.x) * 0.5f - (ImGui::CalcTextSize(cNavBar).x * 0.5f);
                ImGui::SetCursorPos(ImVec2(fTextCenter + 1.f, title_bar_rect.Min.y - window->Pos.y + window->Scroll.y + 1.f));
                ImGui::TextColored(ImVec4(0.f, 0.f, 0.f, 1.f), cNavBar);
                ImGui::SetCursorPos(ImVec2(fTextCenter, title_bar_rect.Min.y - window->Pos.y + window->Scroll.y));
                ImGui::TextColored(ImVec4(1.f, 1.f, 1.f, 1.f), cNavBar);
            }
            IsNavBar = false;
        }
        else if (IsMainMenu)
        {
            // Window
            window->DrawList->AddRectFilled(title_bar_rect.Min, ImVec2(title_bar_rect.Max.x, window->Pos.y + window->Size.y), IM_COL32(33, 33, 33, 255), 5.f);
            window->DrawList->AddRectFilled(title_bar_rect.Min + ImVec2(1.f, 1.f), ImVec2(title_bar_rect.Max.x - 1.f, window->Pos.y + window->Size.y - 1.f), IM_COL32(77, 80, 89, 255), 5.f);
            window->DrawList->AddRectFilled(title_bar_rect.Min + ImVec2(2.f, 2.f), ImVec2(title_bar_rect.Max.x - 2.f, window->Pos.y + window->Size.y - 2.f), IM_COL32(44, 49, 55, 255), 5.f);
            
            // TopBar
            ImVec4 iTop = GetStyleColorVec4(ImGuiCol_TitleBg);
            iTop.x *= 1.05f;
            iTop.y *= 1.05f;
            iTop.z *= 1.05f;
            ImU32 iTop32 = ColorConvertFloat4ToU32(iTop);
            if (IsChildMenu)
            {
                window->DrawList->AddRectFilled(title_bar_rect.Min + ImVec2(1.f, 0.f), ImVec2(title_bar_rect.Max.x - 1.f, title_bar_rect.Min.y + 4.f), iTop32, 2.f, ImDrawCornerFlags_TopLeft | ImDrawCornerFlags_TopRight);
                window->DrawList->AddRectFilled(title_bar_rect.Min + ImVec2(2.f, 1.f), ImVec2(title_bar_rect.Max.x - 2.f, title_bar_rect.Min.y + 3.f), GetColorU32(ImGuiCol_TitleBg), 2.f, ImDrawCornerFlags_TopLeft | ImDrawCornerFlags_TopRight);
                float fTextCenter = (title_bar_rect.Max.x - title_bar_rect.Min.x) * 0.5f - (ImGui::CalcTextSize(window->Name).x * 0.5f);
                ImGui::SetCursorPos(ImVec2(fTextCenter, title_bar_rect.Min.y + 5.f - window->Pos.y + window->Scroll.y));
                ImGui::Text(window->Name);
            }
            else
            {
                window->DrawList->AddRectFilled(title_bar_rect.Min, ImVec2(title_bar_rect.Max.x, title_bar_rect.Min.y + 7.f), iTop32, 5.f, ImDrawCornerFlags_TopLeft | ImDrawCornerFlags_TopRight);
                window->DrawList->AddRectFilled(title_bar_rect.Min + ImVec2(1.f, 1.f), ImVec2(title_bar_rect.Max.x - 1.f, title_bar_rect.Min.y + 6.f), GetColorU32(ImGuiCol_TitleBg), 5.f, ImDrawCornerFlags_TopLeft | ImDrawCornerFlags_TopRight);
                iTop.w = 0.1f;
                window->DrawList->AddRectFilledMultiColor(ImVec2(title_bar_rect.Min.x + 1.f, title_bar_rect.Min.y + 6.f), ImVec2(title_bar_rect.Max.x - 1.f, title_bar_rect.Min.y + 20.f), iTop32, iTop32, IM_COL32(44, 49, 55, 25), IM_COL32(44, 49, 55, 25));
            }
            ...
            if (window->ScrollbarY)
            {
                ...
                const ImGuiID id = GetWindowScrollbarID(g.CurrentWindow, ImGuiAxis_Y);
                KeepAliveID(id);

                ImRect bb = GetWindowScrollbarRect(g.CurrentWindow, ImGuiAxis_Y);
                float size_avail = g.CurrentWindow->InnerRect.Max[ImGuiAxis_Y] - g.CurrentWindow->InnerRect.Min[ImGuiAxis_Y];
                float size_contents = g.CurrentWindow->ContentSize[ImGuiAxis_Y] + g.CurrentWindow->WindowPadding[ImGuiAxis_Y] * 2.0f;
                PushStyleColor(ImGuiCol_ScrollbarBg, IM_COL32(32, 33, 36, 255));
                if (IsChildMenu)
                {
                    bb.Min.x -= 8.f;
                    bb.Max.x -= 8.f;
                ...
                ScrollbarEx(bb, id, ImGuiAxis_Y, &window->Scroll[ImGuiAxis_Y], size_avail, size_contents, 0);
                PopStyleColor();
            }
            IsMainMenu = false;
            IsChildMenu = false;
...
{
    if (!(flags & ImGuiWindowFlags_NoTitleBar)) IsMainMenu = true;
    if (flags & ImGuiWindowFlags_NoBackground) IsChildMenu = true;
    if (flags & ImGuiWindowFlags_NoScrollbar) IsNavBar = true;
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
